...
def get_ai_feedback(scenario, analysis_context, action, raw_data_summary):
    try:
        ...
        CX 매니저의 주요 업무는 인입된 고객 VOC 분석 결과를 바탕으로 타 부서와의 협업을 통해 고객 CX를 개선하고, LTV와 리텐션을 제고하는 유저 리서치 직무입니다.
        ...
        3. 잘 작성된 부분은 근거를 밝혀 말씀하시고, 개선이 필요한 지점 위주로 실무적인 톤으로 작성하세요.
...
def generate_next_background(prev_action, prev_feedback):
    ...
    단, 개별 소비자 대상으로 수행된 액션과 코호트 대상으로 수행된 액션은 반드시 구분하고, 액션의 맥락을 구체적으로 이해한 후 배경을 작성해주세요.
    각 문장은 줄바꿈처리해서 가독성을 올려 출력하세요.
